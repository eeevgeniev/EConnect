...
{
    /// <summary>
    /// Class for parsing Nullable Char from DbDataReader, the Nullable Char the value must be contained in the first field column (index 0) in the DbDataReader.
    ...
    /// </summary>
    internal sealed class NullableCharParser : BaseParser, IParser<char?>
    {
        /// <summary>
        /// Returns IEnumerable where the result is Nullable Char, only the first column is checked.
        /// </summary>
        /// <param name="dbDataReader">DbDataReader, must be not null.</param>
        /// <returns>IEnumerable where the result is Nullable Char.</returns>
        public IEnumerable<char?> Parse(DbDataReader dbDataReader)
        {
            ...
            {
                yield return (!dbDataReader.IsDBNull(0) ? CharsHelperParser.GetNullableCharFromString(dbDataReader, 0) : defaultValue);
        ...
        /// <summary>
        /// Returns only one result from the DbDataReader, the Nullable Char value must be contained in the first field column (index 0) in the DbDataReader.
        ...
        public (bool hasResult, char? result) ParseSingle(DbDataReader dbDataReader)
        {
            ...
            return (dbDataReader.Read() && dbDataReader.FieldCount > 0) ?
                (true, !dbDataReader.IsDBNull(0) ? CharsHelperParser.GetNullableCharFromString(dbDataReader, 0) : defaultValue) :
                (false, defaultValue);
        }
        ...
        public Type Type() => BaseTypeHelper.NullableCharType;
    }
}