...
{
    /// <summary>
    /// Class for parsing Guid from DbDataReader, the Guid the value must be contained in the first field column (index 0) in the DbDataReader.
    /// For every record only one the first field column is checked.
    /// </summary>
    internal sealed class GuidParser : BaseParser, IParser<Guid>
    {
        /// <summary>
        /// Returns IEnumerable where the result is Guid, only the first column is checked.
        /// </summary>
        /// <param name="dbDataReader">DbDataReader, must be not null.</param>
        /// <returns>IEnumerable where the result is Guid.</returns>
        ...
        /// <summary>
        /// Returns only one result from the DbDataReader, the Guid value must be contained in the first field column (index 0) in the DbDataReader.
        ...
        public Type Type() => BaseTypeHelper.GuidType;
    }
}