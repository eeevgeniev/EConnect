...
{
    /// <summary>
    /// Class for parsing UShort from DbDataReader, the UShort the value must be contained in the first field column (index 0) in the DbDataReader.
    /// For every record only one the first field column is checked.
    /// </summary>
    internal sealed class UShortParser : BaseParser, IParser<ushort>
    {
        /// <summary>
        /// Returns IEnumerable where the result is UShort, only the first column is checked.
        /// </summary>
        /// <param name="dbDataReader">DbDataReader, must be not null.</param>
        /// <returns>IEnumerable where the result is UShort.</returns>
        ...
        /// <summary>
        /// Returns only one result from the DbDataReader, the UShort value must be contained in the first field column (index 0) in the DbDataReader.
        ...
        public Type Type() => BaseTypeHelper.UShortType;
    }
}
