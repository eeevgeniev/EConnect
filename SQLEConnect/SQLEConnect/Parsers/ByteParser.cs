...
{
    /// <summary>
    /// Class for parsing Byte from DbDataReader, the Byte the value must be contained in the first field column (index 0) in the DbDataReader.
    /// For every record only one the first field column is checked.
    /// </summary>
    internal sealed class ByteParser : BaseParser, IParser<byte>
    {
        /// <summary>
        /// Returns IEnumerable where the result is Byte, only the first column is checked.
        /// </summary>
        /// <param name="dbDataReader">DbDataReader, must be not null.</param>
        /// <returns>IEnumerable where the result is Byte.</returns>
        public IEnumerable<byte> Parse(DbDataReader dbDataReader)
        {
            ...
            {
                yield return (!dbDataReader.IsDBNull(0) ? dbDataReader.GetByte(0) : defaultValue);
        ...
        /// <summary>
        /// Returns only one result from the DbDataReader, the Byte value must be contained in the first field column (index 0) in the DbDataReader.
        ...
        public (bool hasResult, byte result) ParseSingle(DbDataReader dbDataReader)
        {
            ...
            return (dbDataReader.Read() && dbDataReader.FieldCount > 0) ?
                (true, (!dbDataReader.IsDBNull(0) ? dbDataReader.GetByte(0) : defaultValue)) :
                (false, defaultValue);
        }
        ...
        public Type Type() => BaseTypeHelper.ByteType;
    }
}