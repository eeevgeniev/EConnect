...
namespace SQLEConnectTests.SQLServerQueries
{
    class Program
    {
            ...
            for (int i = 0; i < results.Count; i++)
            {
                ...
                else if (i == 1)
                {
                    foreach (string key in keys.Keys)
                    {
                        if (string.Equals(key, nameof(GenericTable.ByteNumber), StringComparison.OrdinalIgnoreCase))
                        {
                            Debug.Assert((byte)keys[key] == Constants.ByteNumber2);
                        }
                        else if (string.Equals(key, nameof(GenericTable.NullableByteNumber), StringComparison.OrdinalIgnoreCase))
                        {
                            Debug.Assert((byte?)keys[key] == Constants.NullableByteNumber2);
                        }
                        else if (string.Equals(key, nameof(GenericTable.BoolValue), StringComparison.OrdinalIgnoreCase))
                        {
                        ...
                        {
                            Debug.Assert((Guid)keys[key] == Constants.GuidValue2);
                        }
                        else if (string.Equals(key, nameof(GenericTable.NullableGuidValue), StringComparison.OrdinalIgnoreCase))
                        {
                            Debug.Assert((Guid?)keys[key] == Constants.NullableGuidValue2);
                        }
                        else if (string.Equals(key, nameof(GenericTable.IntNumber), StringComparison.OrdinalIgnoreCase))
                        {
                        ...
                        {
                            Debug.Assert((byte)keys[key] == Constants.SByteNumber2);
                        }
                        else if (string.Equals(key, nameof(GenericTable.NullableSByteNumber), StringComparison.OrdinalIgnoreCase))
                        {
                            Debug.Assert((byte?)keys[key] == Constants.NullableSByteNumber2);
                        }
                        else if (string.Equals(key, nameof(GenericTable.ShortNumber), StringComparison.OrdinalIgnoreCase))
                        {
        ...
        private static void CheckSingleDict(Dictionary<string, object> dict)
        {
            foreach (string key in dict.Keys)
            {
                if (string.Equals(key, nameof(GenericTable.ByteNumber), StringComparison.OrdinalIgnoreCase))
                {
                    Debug.Assert((byte)dict[key] == Constants.ByteNumber2);
                }
                else if (string.Equals(key, nameof(GenericTable.NullableByteNumber), StringComparison.OrdinalIgnoreCase))
                {
                    Debug.Assert((byte?)dict[key] == Constants.NullableByteNumber2);
                }
                else if (string.Equals(key, nameof(GenericTable.BoolValue), StringComparison.OrdinalIgnoreCase))
                {
                ...
                {
                    Debug.Assert((Guid)dict[key] == Constants.GuidValue2);
                }
                else if (string.Equals(key, nameof(GenericTable.NullableGuidValue), StringComparison.OrdinalIgnoreCase))
                {
                    Debug.Assert((Guid?)dict[key] == Constants.NullableGuidValue2);
                }
                else if (string.Equals(key, nameof(GenericTable.IntNumber), StringComparison.OrdinalIgnoreCase))
                {
                ...
                {
                    Debug.Assert((byte)dict[key] == Constants.SByteNumber2);
                }
                else if (string.Equals(key, nameof(GenericTable.NullableSByteNumber), StringComparison.OrdinalIgnoreCase))
                {
                    Debug.Assert((byte?)dict[key] == Constants.NullableSByteNumber2);
                }
                else if (string.Equals(key, nameof(GenericTable.ShortNumber), StringComparison.OrdinalIgnoreCase))
                {
        ...
        {
            using (Connection<SqlConnection> connection = new Connection<SqlConnection>(connectionString, true))
            {
                connection.NonQuery(@"
                            IF NOT EXISTS (SELECT * FROM sys.tables t JOIN sys.schemas s ON (t.schema_id = s.schema_id) WHERE s.name = 'dbo' AND t.name = 'GenericTable') 	
                            ...
                            (
                                ...
                                Bytes VARBINARY(50) NULL,
                                Chars NVARCHAR(MAX) NULL,
                                Letter NCHAR(1) NULL,
                                NullableLetter NCHAR(1) NULL,
                                DateTimeValue DATETIME2 NULL,
                                NullableDateTimeValue DATETIME2 NULL,
                                DecimalNumber DECIMAL(19, 5) NULL,
                                NullableDecimalNumber DECIMAL(19, 5) NULL,
                                DoubleNumber FLOAT NULL,
                                NullableDoubleNumber FLOAT NULL,
                                FloatNumber FLOAT(24) NULL,
                                NullableFloatNumber FLOAT(24) NULL,
                                GuidValue UNIQUEIDENTIFIER NULL,
                                NullableGuidValue UNIQUEIDENTIFIER NULL,
                                IntNumber INT NULL,
                                NullableIntNumber INT NULL,
                                LongNumber BIGINT NULL,
                                NullableLongNumber BIGINT NULL,
                                SByteNumber TINYINT NULL,
                                NullableSByteNumber TINYINT NULL,
                                ShortNumber SMALLINT NULL,
                                NullableShortNumber SMALLINT NULL,
                                StreamValue VARBINARY(50) NULL,
